    ...
    def register(self):
        ...
        if register_user(user_id, password):
            popup = Popup(title='Registration Successful',
                          content=Label(text='Registration successful! You can now log in.'), size_hint=(0.6, 0.4))
            popup.open()
        else:
            popup = Popup(title='Registration Failed', content=Label(text='User ID already exists'),
                          size_hint=(0.6, 0.4))
            popup.open()
